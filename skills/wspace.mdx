...
The `wspace` skill encodes the workspace protocol for multi-repo development.
It covers worktree isolation, pipeline execution, goal-loop automation, and
agent coordination — the opinionated practices that apply to any multi-repo
workspace built on `wspace`.
