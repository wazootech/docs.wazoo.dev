...
The `wazoo` skill packages Wazoo's control plane and data plane features into a skill interface compatible with AI coding agents (Claude Code, Cursor, OpenCode, Gemini, etc.).
...
- **World Management**: Create, view, list, update, delete, and undelete memory worlds.
- **Token Management**: Issue and revoke API tokens for scoped application access.

### Data Plane Operations
- **Graph Import**: Add Turtle, JSON-LD, N-Triples, or N-Quads data into active worlds.
- **Hybrid Search**: Query memory using combined vector similarity and pattern matching filters.
- **Graph Pattern Queries (SPARQL)**: Execute queries for deterministic knowledge graph retrieval.
