...
- **Conservative operations**: Mutating command (`update`) refuses to rewrite
  user history, touch dirty repositories, or alter unmanaged checkouts.
...
The JSR package installs a binary named `wspace`. Building from source writes
...
1. **Scaffold**: `wspace init` (new workspaces) or `wspace install` (existing
   manifests)
...
5. **Develop**:
   ...
   ```
6. **Sync local secrets**: copy local credentials (`.env` files, tokens) into
   the checkout manually; they are never committed or synced by `wspace`
...
- **Manifest discovery**: By default `wspace` looks for `workspace.json`, then
...
- **Directories**: `repositoriesDirectory` defaults to `repos`. Relative paths
  anchor to the folder containing the manifest file.
