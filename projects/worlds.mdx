...
- The Worlds Data API, hosted at `data.wazoo.dev`, handles graph data through
  import, export, search, and SPARQL operations. Its generated TypeScript client
  is `@worlds/client`; the embedded `@worlds/sdk` exposes the same operations
  in-process.
...
```bash
curl -s -X POST "https://data.wazoo.dev/worlds/quickstart-cloud/import" \
...
```bash
curl -s -X POST "https://data.wazoo.dev/worlds/quickstart-cloud/search" \
  -H "Authorization: Bearer $WORLDS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "worlds",
...
```bash
curl -s -X POST "https://data.wazoo.dev/worlds/quickstart-cloud/sparql" \
  -H "Authorization: Bearer $WORLDS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
