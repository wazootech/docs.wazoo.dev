...
The connection model uses composable components to keep system complexity
manageable:
...
- Platform management: Use the [Platform API](/platform/api) or `@wazoo/client`
  to manage world metadata, platform tokens, world auth tokens, and usage
  records.
...
<Info>
  ...
  stay with management systems and trusted automation. For the full credential
  model, see [Security](/contribute/security).
</Info>
...
```mermaid
graph LR
    %% Class definitions for accessibility & brand colors
    classDef engine fill:#161617,stroke:#FF9800,stroke-width:2px,color:#ffffff;
    classDef persistence fill:#1f1f20,stroke:#F57C00,stroke-width:2px,color:#ffffff;
    classDef process fill:#282829,stroke:#FFB74D,stroke-width:1.5px,color:#ffffff;
    ...
    class Dev,Agent process;
    class API,ConsoleView,Auth,Engine,Sync engine;
    class DB persistence;
```

## SDK topology

Wazoo generates clients from OpenAPI specs to ensure type safety.
...
flowchart TD
    %% Class definitions for accessibility & brand colors
    classDef api fill:#161617,stroke:#FF9800,stroke-width:2px,color:#ffffff;
    classDef sdk fill:#1f1f20,stroke:#F57C00,stroke-width:2px,color:#ffffff;
    classDef apps fill:#282829,stroke:#FFB74D,stroke-width:1.5px,color:#ffffff;

    APIServer[Wazoo API Server<br/>api.wazoo.dev]
    OpenAPI[Wazoo OpenAPI Spec<br/>/openapi.json]
    SDK[Wazoo SDK<br/>@wazoo/client]
    Console[Wazoo Console<br/>Next.js]
    CLI[Wazoo CLI]
    Desktop[Wazoo Desktop]
    Tools[Wazoo AI Tools<br/>@wazoo/tools]

    APIServer -->|exposes| OpenAPI
    OpenAPI -->|hey-api codegen| SDK
    SDK -->|powers| Console
    SDK -->|powers| CLI
    SDK -->|powers| Desktop
    SDK -->|powers| Tools
    ...
    class APIServer,OpenAPI api;
    class SDK sdk;
    class Console,CLI,Desktop,Tools apps;
```
...
flowchart TD
    ...
    WorldsAPI[Worlds API Server<br/>worlds-api.wazoo.dev]
    OpenAPI[Worlds OpenAPI Spec<br/>/openapi.json]
    HTTPBackend[Generated HTTP Backend]
    WorldsSDK[Worlds SDK<br/>@worlds/client]
    CustomBackends[Custom Backends<br/>N3 · Sqlite · Postgres]
    Console[Wazoo Console<br/>Next.js]
    CLI[Wazoo CLI]
    Desktop[Wazoo Desktop]
    Tools[Wazoo AI Tools<br/>@wazoo/tools]

    WorldsAPI -->|exposes| OpenAPI
    OpenAPI -->|hey-api codegen| HTTPBackend
    HTTPBackend -.->|implements| WorldsSDK
    CustomBackends -.->|implements| WorldsSDK
    WorldsSDK -->|powers| Console
    WorldsSDK -->|powers| CLI
    WorldsSDK -->|powers| Desktop
    WorldsSDK -->|powers| Tools
    HTTPBackend -->|REST| WorldsAPI

    class WorldsAPI,OpenAPI api;
    class HTTPBackend,WorldsSDK,CustomBackends sdk;
    class Console,CLI,Desktop,Tools apps;
```
...
The platform management layer runs as a Cloudflare Workers API on Hono with
Turso/libSQL for metadata. It records users, Worlds, tokens, usage, and limits.
...
<Note>
  Some deployment modes are in active development and not publicly available
  yet.
</Note>
...
flowchart TD
    ...
    classDef engine fill:#161617,stroke:#FF9800,stroke-width:2px,color:#ffffff;
    classDef persistence fill:#1f1f20,stroke:#F57C00,stroke-width:2px,color:#ffffff;
    classDef process fill:#282829,stroke:#FFB74D,stroke-width:1.5px,stroke-dasharray: 5 5,color:#ffffff;
...
The platform uses a WASM-compiled RDF store that runs entirely within the
JavaScript runtime. This maintains an in-memory state in the edge cache between
requests to reduce read latency.

### Persistence and indexing

Persistence uses an edge-distributed database to maintain semantic integrity.
The system relies on a multi-index strategy:
...
When executing a search, the engine uses Reciprocal Rank Fusion (RRF) to combine
results from the vector index and full-text index into a single relevance
ranking. Structural graph constraints further restrict these results.
...
Wazoo uses TypeScript across the public clients and platform management surface.
The Platform API server runs on Cloudflare Workers with Hono and Turso/libSQL
metadata. Worlds data-plane packages are published separately and may target
different JavaScript runtimes depending on the adapter.

## Package topology

The public surface is split across focused packages. The important boundary is
between platform management packages and Worlds data-plane packages.

```mermaid
graph TD
    PLATFORM_SERVER["wazoo-api<br/>Platform API server"]
    PLATFORM_SDK["@wazoo/client<br/>Platform SDK"]
    WORLDS_CLIENT["@worlds/client<br/>Worlds data client"]
    WORLDS_TOOLS["@wazoo/tools<br/>Agent tools"]
...
The Platform API is organized by management resource:
...
Worlds uses a standardized core ontology to provide agents with a predictable
set of primitives for reasoning, alignment, and discovery.
...
Deployment modes may not be publicly available yet.

The Worlds API combines symbolic precision with the statistical power of large
language models. Reification and human-feedback alignment provide a
deterministic substrate for intentional agency in high-stakes contexts.
