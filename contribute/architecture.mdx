...
public-facing Platform API manages organizations, world metadata, tokens, usage,
limits, and billing state. The Worlds Data API handles graph operations such as
RDF import, RDF export, search, and SPARQL.
...
- Platform management: Use the [Platform API](/platform/api) or `@wazoo/api` to
  manage organizations, world metadata, platform tokens, world auth tokens, and
  usage records.
...
manages organizations, world records, tokens, usage, limits, and billing state.
...
Hono and Turso/libSQL for metadata. It currently records organizations, worlds,
tokens, usage, and limits. The full production provisioning flow between
platform metadata and data-plane services is still evolving.

The control database uses AIP-aligned terminology: public immutable IDs are
stored as `organization_id` and `world_id`, mutable labels as `display_name`,
and internal support identities as `uid`. Public resource names are computed
from the public IDs and never from internal UIDs.
...
Turso/libSQL database per World and `:sync` reconciles the deterministic database
and schema metadata.
...
- Isolation: Access Worlds by public World ID or canonical resource name to ensure zero
