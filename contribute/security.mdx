...
against the Wazoo Platform API, including organizations, world metadata,
...
the active token count or access duration when an organization is throttled.
...
use `kind = "ADMIN"`, `organization_uid = NULL`, and include the `admin` scope.
