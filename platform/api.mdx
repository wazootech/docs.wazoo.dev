...
Until full Turso provisioning is wired end-to-end, newly created Worlds can show
`provisioning.state = "PENDING"`; this means platform metadata exists but the
per-World Turso database has not been verified by the Platform API yet.

`POST /v1/organizations/:organizationId/worlds/:worldId:sync` returns a
`syncReport`. Before real reconciliation ships, the report is explicit when
Turso provisioning is not implemented instead of pretending the World is healthy.
