...
### LLM-consumable documentation mechanics

Wazoo documentation is published for both human developers and AI coding agents:

- **`llms.txt` & `llms-full.txt`**: Automatically generated by Mintlify to index
  `AGENTS.md`, `SKILL.md`, and all docs pages.
- **Per-page `.md` aliases**: Mintlify serves raw markdown variants for pages.
  Verify that new pages render cleanly without broken formatting when queried as
  `.md`.
- **Agent affordances**: Maintain raw markdown accessibility across all
  navigation structures and repository entries.
