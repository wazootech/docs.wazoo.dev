...
delegates those repositories to the cluster. `wspace` still sees every
repository as one resolved tree, while scoped commands operate on a single
cluster.

### Declare a sub-workspace

Add a `workspaces:` block to the root `repos.json` and move the cluster's
repositories out of the root list. Each entry names the sub-workspace and points
at its child manifest:
...
```json
{
  ...
  "workspaces": [
    {
      "name": "umbra-suite",
      "path": "repos/umbra-wiki/workspace.json"
    }
  ]
}
```
...
manifests can declare their own `workspaces:` entries to nest further. Circular
references and duplicate sub-workspace names raise errors.
...
Omitting `--workspace` operates on the full resolved tree. Manifests without a
`workspaces:` block behave exactly as before.

<Note>
  Workspace nesting and wiki composition solve different problems: the
  `workspaces:` block organizes Git repositories, while transitive `sources:`
  blocks compose RDF corpora. One umbrella can install another umbrella's
  published corpus with no workspace nesting, and a sub-workspace can hold
  several umbrella wikis whose graphs never merge.
</Note>
