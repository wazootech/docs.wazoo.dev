...
description:
  ...
  those repositories as one multi-repo workspace with works."
---
...
manage those repositories as one multi-repo workspace with `works`.
...
repository. `works` keeps the repositories in sync with `check`, `update`, and
worktree workflows.
...
- The `works` CLI, installed per the
...
manifest so `works` manages them as a workspace:

```text
workspace-root/
  wiki.yml          # umbrella: inputs + sources
  workspace.json     # works manifest
  repos/
...
<Steps>
  <Step title="Declare the repositories in the manifest">
    ...
    `works check` confirms every repository is present and clean. Keep the
  ...
  <Step title="Keep repositories and content in sync">
    ...
    `works update` fast-forwards clean default branches and skips dirty or
...
checked-out repository can carry one at its root. Run `works` from that
...
workspace and the sub-workspace are independent: `works` commands run in the
