...
The `SparqlRequest` carries `timeoutMs` and an `AbortSignal`. If you want to
honour them, compose an `AbortController` that fires on whichever comes first:
...
custom engine. The built-in `WazooSparqlEngine` includes this out of the box.
