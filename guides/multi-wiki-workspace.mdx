...
A single wiki is right for one team or one domain. As teams and domains move
independently, splitting into separate wikis keeps ownership boundaries intact —
but a house of isolated wikis loses the ability to query them as one corpus.
This guide composes an umbrella wiki from independent sub-wiki repositories, and
manages those repositories as one multi-repo workspace with `wspace`.

The umbrella wiki declares each sub-wiki as an external `sources:` entry in its
`wiki.yml`, pins exact commits in `wiki.lock`, and keeps each sub-wiki as a
named RDF graph so you can query the whole corpus or trace any result to its
source. `wspace` keeps the repositories in sync with `check`, `update`, and
worktree workflows.
...
- **Team autonomy**: each team owns its wiki history and merges its own changes.
- **Reusable corpora**: a reference library (product docs, regulations, shared
  domain models) reused across multiple umbrellas.
- **Independent cadence**: sub-wikis that publish or evolve on their own
  schedule.

Keep one wiki when the corpus is small or static, or when every page changes at
the same pace. Splitting adds a composition layer; a single wiki is still the
right default.
...
repositories — umbrella and sub-wikis — are listed in one `repos.json` manifest
so `wspace` manages them as a workspace:
...
<Steps>
  ...
  <Step title="Compose the sub-wikis in wiki.yml">
    ...
    Use a git URL with a `#ref` pin — branch, tag, or commit — so the
    ...
    <Note>
      ...
      [wiki install](https://github.com/wazootech/wiki/blob/main/docs/wiki/wiki_install.md)
      and
      [External data sources (`sources:`)](https://github.com/wazootech/wiki/blob/main/docs/wiki/Wiki_Configuration.md#external-data-sources-sources).
      For the product framing, see
      [Recursive Semantic Datasets](https://github.com/wazootech/wiki/blob/main/docs/wiki/Recursive_Semantic_Datasets.md).
    </Note>
  ...
  <Step title="Query the composed corpus with per-source provenance">
    ...
    <Note>
      ...
      [wiki graph](https://github.com/wazootech/wiki/blob/main/docs/wiki/wiki_graph.md).
    </Note>
