...
- `SqliteSearchIndex` (durable): Full hybrid search with FTS5 keyword search
  and sqlite-vec vector search. Supports Reciprocal Rank Fusion (RRF) to
  combine both signals. Requires `@worlds/sqlite` and an `EmbeddingService`.
...
const client: SdkInterface = new Sdk({
...
The `SqliteSearchIndex` supports full hybrid search with vector embeddings.
This enables semantic similarity search where natural language queries find
...
// For production: use AiSdkEmbeddingService with a real embedding model
// import { AiSdkEmbeddingService } from "@worlds/sdk/ai-sdk";
// import { createGoogleGenerativeAI } from "@ai-sdk/google";

// Testing: use FakeEmbeddingService (no API key needed)
// Production: swap to AiSdkEmbeddingService for real embeddings
const client = await createSqliteSdk({
  path: ":memory:",
  embeddingService: new FakeEmbeddingService(),
  vectorDimensions: 32,  // FakeEmbeddingService uses 32 dimensions
...
The neural network finds the starting point through semantic similarity.
SPARQL retrieves the exact facts. No guessing.

Note: The example above uses `FakeEmbeddingService` for testing without an API
key. With a real embedding service (like `AiSdkEmbeddingService`), the vector
branch contributes semantic similarity scores, and the RRF fusion combines both
keyword and vector signals for better ranking.
...
import { RdfjsQuadStore, RdfjsSearchIndex } from "@worlds/sdk/rdfjs";
import { MemoryStore, WazooSparqlEngine } from "@wazoo/sparql-engine";
import { createTools } from "@worlds/sdk/ai-sdk";
