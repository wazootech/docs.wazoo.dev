...
// For cloud embeddings: use AiSdkEmbeddingService with a real embedding model
// import { AiSdkEmbeddingService } from "@worlds/sdk/ai-sdk";
// import { createGoogleGenerativeAI } from "@ai-sdk/google";

// TF.js Universal Sentence Encoder: local embeddings, no API key needed
// Produces 512-dimensional vectors for semantic similarity search
const client = await createSqliteSdk({
  path: ":memory:",
  embeddingService: new UniversalSentenceEncoderEmbeddingService(),
  vectorDimensions: 512,  // USE produces 512-dimensional vectors
...
Note: The example above uses TF.js Universal Sentence Encoder for local
embeddings (no API key needed). For cloud embeddings, swap to
`AiSdkEmbeddingService` with a model like `text-embedding-004`.
