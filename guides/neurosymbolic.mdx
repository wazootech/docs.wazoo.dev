...
### Hybrid search: neural + SPARQL

Search uses two steps:

- Neural fuzzy search discovers the starting point
...
// Step 1: Neural fuzzy search (discovery)
const searchResults = await client.search({
  query: "Who does Ethan work for?",
});
...
The neural network finds the starting point. SPARQL retrieves the exact facts.
No guessing.
