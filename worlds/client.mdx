...
from the canonical
[Worlds Data API OpenAPI document](https://data.wazoo.dev/openapi.json). It
mirrors the management-plane client pattern: `@wazoo/client` talks to the
platform API, `@worlds/client` talks to the data plane.

Use it for remote data-plane operations: world lifecycle, import, export,
search, SPARQL, and reindexing against `data.wazoo.dev`.
