...
- Wazoo Worlds applies the same RDF graph model to agent memory, so agents can
  query connected entities with SPARQL instead of relying on flat vector
  embeddings.
...
- Agents use web search tools (Google Search API, Serper, Tavily) to retrieve
  live, external information from the open web.
