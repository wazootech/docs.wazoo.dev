...
The **LLM Wiki pattern** (popularized by Andrej Karpathy and demonstrated in
project Farzapedia) and **Wazoo Worlds** share the exact same core philosophy:
**agent memory should be compiled, structured, and interlinked rather than
dumped into flat, opaque vector databases.**
...
- **LLM Wiki** defines the human-and-agent readable workspace—where agents
  ...
  (`[[wikilinks]]`).
- **Worlds** provides the underlying **schema-checked fact engine**—taking those
  compiled articles and frontmatter assertions, enforcing SHACL shape
  validation, and allowing agents to execute SPARQL graph queries across the
  knowledge graph.
