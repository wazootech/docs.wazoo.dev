...
MemMachine is an open-source memory service that splits its model in two.
Episodic memory, the relational context of conversations, lives in a Neo4j
graph, while profile data lives in SQL. Retrieval walks the graph to reconstruct
who said what to whom. It emphasizes token efficiency and works across LLM
providers, including AWS Bedrock.
